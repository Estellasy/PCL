...
from pcl.loss import SinkhornSim
...
def train(train_loader, model, criterion, optimizer, epoch, args, cluster_result_global=None, cluster_result_dense=None):
    ...
    for i, (images, index) in enumerate(train_loader):
        ...
        # compute output
        result = model(im_q=images[0], im_k=images[1], cluster_global=cluster_result_global,
                       cluster_dense=cluster_result_dense, index=index)
